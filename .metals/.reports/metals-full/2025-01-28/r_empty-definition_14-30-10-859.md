error id: _empty_/Szef#receive().$anonfun().drugaGrupa.
file:///C:/Users/JSadr/Desktop/studia2/Scala/kolos_2_202x_zad1/src/main/scala/Main.scala
empty definition using pc, found symbol in pc: _empty_/Szef#receive().$anonfun().drugaGrupa.
...
class Szef extends Actor with ActorLogging {
  ...
  def przyjmowanieOdpowiedzi(kandydaci: Set[ActorRef], pierwszaGrupa: Set[ActorRef], drugaGrupa: Set[ActorRef], bazaDanychOdpowiedzi: Map[Int, ActorRef], licznik: Int): Receive = {
    case Odpowiedz(odp) =>
      val updatedBazaDanychOdpowiedzi = bazaDanychOdpowiedzi + ((if (pierwszaGrupa.contains(sender())) 1 else 2, sender()))
      if (licznik + 1 == pierwszaGrupa.size + drugaGrupa.size) {
        val najlepszyPierwszaGrupa = pierwszaGrupa.maxBy(kand => updatedBazaDanychOdpowiedzi.count(_ == (1, kand)))
        val najlepszyDrugaGrupa = drugaGrupa.maxBy(kand => updatedBazaDanychOdpowiedzi.count(_ == (2, kand)))
        log.info(s"Wybralem $najlepszyPierwszaGrupa oraz $najlepszyDrugaGrupa")
